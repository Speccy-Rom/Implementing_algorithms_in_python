...
def get_code(root, codes=dict(), code=''):
    ...
    get_code(root.left, codes, f'{code}0')
    get_code(root.right, codes, f'{code}1')
...
def get_tree(string):
    ...
    if len(string_count) <= 1:
        ...
        if len(string_count) == 1:
            node.left = Node(list(string_count)[0])
    ...
    while len(string_count) != 1:
        ...
        node.left = Node(spam[0][0]) if isinstance(spam[0][0], str) else spam[0][0]
        node.right = Node(spam[1][0]) if isinstance(spam[1][0], str) else spam[1][0]
        del string_count[spam[0][0]]
        del string_count[spam[1][0]]
        string_count[node] = spam[0][1] + spam[1][1]

    return list(string_count)[0]


def coding(string, codes):
    return ''.join(codes[symbol] for symbol in string)
